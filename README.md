...
- [done] Nanomancy step now mirrors OBR card workflow: `Current ND`, `Nanomantic Effects Known` cap, `Add Effect` modal (full effect description), per-effect cards with `Use`/`Remove`, per-effect range/actions/counter controls, and shared bonus/penalty dice application.
